...
def validate_args() -> tuple[str, bool] | tuple[None, bool]:
	...
	parser.add_argument("-d", "--details", type=bool, help="Print details", default=False, const=True, nargs='?')
...
def reduce(left: dict, right: dict) -> dict:
	...
	reduced_terms = {degree: coefficient for degree, coefficient in reduced_terms.items() if coefficient != 0}
	# print("reduced_terms: ", reduced_terms)
	if not reduced_terms:
		return {0: 0}
	for degree in reduced_terms:
		coefficient = reduced_terms[degree]
		coefficient_str = "{:.0f}".format(coefficient) if coefficient.is_integer() else str(coefficient)
		if coefficient == 0:
			continue
		if degree == 0:
			if coefficient != 0:
			...
			if coefficient == 1:
				...
				reduced_equation += ((coefficient_str + " * " if coefficient != 1 else "") + "X ^ " + str(degree))
			reduced_equation += " + " if len(reduced_terms) > 1 else ""  # TODO: remove trailing +
	reduced_equation += " = 0"
...
def print_imaginary_number(imaginary_number: complex) -> None:
	...
	"""
	real_part = "{:.6f}".format(imaginary_number.real).rstrip('0').rstrip('.') if imaginary_number.real != 0 else ""
	imaginary_part = "{:+.6f}i".format(imaginary_number.imag).rstrip('0').rstrip('.') if imaginary_number.imag != 0 else ""
	formatted_result = "{}{}".format(real_part, imaginary_part)
	if formatted_result == "":
		formatted_result = "0"
...
def solve_degree_2(details: bool, equation_terms: dict) -> bool:
	...
	print_with_details(details, "Δ = ", "b² - 4ac = ", parentheses(b), "² - 4 * ", parentheses(a), " * ",
																				parentheses(c), " = ", discriminant, "\n")
	if discriminant < 0:
		print("Discriminant is strictly negative, the two solutions are:")
		print_with_details(details, "z1 = (-b - i√|Δ|) / 2a = (-", parentheses(b), " - i√|", discriminant,
																					"|) / ", 2 * a, " = ")
		res_1 = (-b - discriminant ** 0.5) / (2 * a)
		print_imaginary_number(res_1)
		print_with_details(details, "z2 = (-b + i√|Δ|) / 2a = (-", parentheses(b), " + i√|", discriminant,
																					"|) / ", 2 * a, " = ")
		res_2 = (-b + discriminant ** 0.5) / (2 * a)
		print_imaginary_number(res_2)
	elif discriminant == 0:
		print("Discriminant is zero, the solution is:")
		print_with_details(details, "x = (-b - √|Δ|) / 2a = (-", parentheses(b), " - √|", discriminant,
																					"|) / ", 2 * a, " = ")
		res = (-b - discriminant ** 0.5) / (2 * a)
		print_float(res)
	else:
		print("Discriminant is strictly positive, the two solutions are:")
		print_with_details(details, "x1 = (-b - √|Δ|) / 2a = (-", parentheses(b), " - √|", discriminant,
																					"|) / ", 2 * a, " = ")
		res_1 = (-b - discriminant ** 0.5) / (2 * a)
		print_float(res_1)
		print_with_details(details, "x2 = (-b + √|Δ|) / 2a = (-", parentheses(b), " + √|", discriminant,
																					"|) / ", 2 * a, " = ")
		res_2 = (-b + discriminant ** 0.5) / (2 * a)
		print_float(res_2)
	return True
...
def solve_degree_1(details: bool, equation_terms: dict) -> bool:
	...
	print_with_details(details, "x = -b / a = -", parentheses(b), " / ", parentheses(a), " = ")
	res = -b / a
	print_float(res)
	return True
...
if __name__ == "__main__":
	...
	# print("left_equation_part: ", left_equation_part)
	...
	# print("equation_dict: ", equation_dict)
